...
class SubstitutionCipher:
    def __init__(self):
        self.alphabet = ("abcdefghijklmnopqrstuvwxyz" + "0123456789" + "!@#$%^&*()_+-=[]{}|;:',.<>/?`~" + " " + "ABCDEFGHIJKLMNOPQRSTUVWXYZ")
        self.key = ("zjrekydnqoluaxmicvpgtfbhws" + "9543682107" + "']^@$!=[~?>_*`;<&/),|#(%+.{:}-" + " " + "ZJREKYDNQOLUAXMICVPGTFBHWS")
       
        if len(self.key) != len(self.alphabet):
            raise ValueError(f"Alphabet length ({len(self.alphabet)}) " f"must match key length ({len(self.key)})")
        if set(self.key) != set(self.alphabet):
            raise ValueError("The key must be a permutation of the alphabet!")
  ...
  # define encryption and decryption
    def encrypt(self, plaintext: str) -> str:
        ...
        result = []
        invalidCharactersCheck = set(plaintext) - set(self.alphabet)
        if invalidCharactersCheck:
            raise ValueError(f"Encrypt function() has received invalid characters: {invalidCharactersCheck}")

        for characters in plaintext:
            plain = characters.lower() 
            if plain in self.alphabet: 
                idx = self.alphabet.index(plain)
                substitution = self.key[idx]
                #result.append(self.key[idx])
                result.append(substitution.upper() if characters.isupper() else substitution)
            else: 
    ...
    def decrypt(self, ciphertext: str) -> str:
        ...
        invalidCharactersCheck = set(ciphertext) - set(self.key)
        if invalidCharactersCheck:
            raise ValueError(f"Decrypt function() has received invalid characters: {invalidCharactersCheck}")
        
    
        result = []
        for characters in ciphertext:
            textLower = characters.lower()
            if textLower in self.key: 
                idx = self.key.index(textLower)
                originalText = self.alphabet[idx]
                result.append(originalText.upper() if characters.isupper() else originalText)
            else: 
                result.append(characters)
        return ''.join(result)
...
def handle_encrypt_decrypt(cipher):
    while True:    
        ...
        print("3. Encrypt File Content")
        print("4. Decrypt File Content")
        print("0. Return Home")
        print("Input and press return/enter for any character besides 1, 2, 3 or 4 to return home")
        ...
        if choice == '1':
            ...
            invalidCharactersCheck = {characters for characters in data if characters not in cipher.alphabet}
            if invalidCharactersCheck:
                print(f"\nError: Input contains invalid characters. {invalidCharactersCheck}")
                continue
            print("\nEncrypted Text: ", cipher.encrypt(data))

        elif choice == '2':
            data = input("Enter the text to decrypt: ")
            invalidCharactersCheck = {characters for characters in data if characters not in cipher.alphabet}
            if invalidCharactersCheck:
                print(f"\nError: Input contains invalid characters. {invalidCharactersCheck}")
                continue
            print("\nDecrypted Text: ", cipher.decrypt(data))  
            ...
            outPutFile = input("Enter the output file name or press enter/return to overwrite the file contents\n").strip()
            if not outPutFile:
                outPutFile = filename

            with open(outPutFile, 'w', encoding='utf-8') as file:
                file.write(encryptedText)
            print(f"Encrypted contents saved to '{outPutFile}'")
            ...
            decryptedTest = cipher.decrypt(data)

            outputFile = input("Enter the output filename or press enter/return to overwrite the file contents.\n").strip()
            if not outputFile:
                outputFile = filename

            with open(outputFile, 'w', encoding='utf-8') as file:
                file.write(decryptedTest)

            
            print(f"Decrypted contents saved to '{outputFile}'")
        elif choice == '0':
            break
